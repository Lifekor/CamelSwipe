...
@router.post('/claim')
async def claim_async(user_id: int, current_path: float, service: PointService = Depends()):
    return await service.claim_async(user_id=user_id, current_path=current_path)
